/**
 * @fileoverview Unified geographic utilities for distance, bearing, and projection calculations.
 * All geo-related computations should use these functions.
 */
 ...
 * Use for accurate measurements (>1km distances).
 *
 ...
 * Uses simplified spherical model - accurate for short distances (<10km).
 */
...
/**
 * Euclidean distance (planar). Use for relative comparisons only.
 */
...
/**
 * Squared Euclidean distance. Use when comparing distances to avoid sqrt.
 */
...
/**
 * Calculates initial bearing (forward azimuth) from A to B.
...
/**
 * Cartesian angle between two points.
 */
...
/**
 * Interpolates between angles, handling 359→0 wrap.
 */
...
export interface SnapResult {
    ...
    angle: number;
    segmentIndex: number;
    /** Normalized progress along segment (0-1) */
    t: number;
}
...
export interface SnapOptions {
    ...
    /** Number of segments to search around hint */
    searchRadius?: number;
    /** Minimum segment index (prevents backward snapping) */
    minSegmentIndex?: number | null;
}

/**
 * Projects a point onto the nearest location on segment AB.
 */
 ...
 * Uses segment hints for performance optimization.
 */
