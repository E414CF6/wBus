/**
 * @fileoverview Unified polyline data service.
 * Handles fetching, transformation, and caching of route polyline data.
 */
...
function splitByTurnIndex(coords: Coordinate[], turnIndex: number): { up: Coordinate[]; down: Coordinate[] } {
    ...
    return {
        up: coords.slice(0, idx + 1),
        down: coords.slice(idx),
    };
}
...
/**
 * Fetches and processes a single route's polyline data.
...
/**
 * Fetches and processes multiple route polylines.
...
/**
 * Processes multiple polylines into deduplicated segments.
 * Useful for rendering overlapping routes efficiently.
 */
