...
import {type BusSchedule, DAY_TYPES, DayType, RowItem} from "@entities/route/types";
...
const dayTypeToLabel = {
    [DAY_TYPES.WEEKDAY]: DAY_LABELS.WEEKDAY, [DAY_TYPES.WEEKEND]: DAY_LABELS.WEEKEND,
} as const;
...
function useScheduleLogic(data: BusSchedule) {
    ...
    const activeSchedule = useMemo(() => (isGeneralSchedule ? data.schedule.general! : data.schedule[dayType]!), [data.schedule, dayType, isGeneralSchedule]);

    // Derived State: Sorted list of hours
    const hours = useMemo(() => Object.keys(activeSchedule).sort(), [activeSchedule]);

    // Derived State: Next bus info
    const nextBus = useMemo(() => findNextBus(activeSchedule, hours, resolvedDirection, now), [activeSchedule, hours, resolvedDirection, now]);
...
const DirectionSelector = ({
                               directions, current, onChange, isCompact
                           }: {
    directions: string[]; current: string; onChange: (d: string) => void; isCompact: boolean;
...
const TimetableGrid = ({
                           hours, highlightedHour, schedule, direction
                       }: {
    hours: string[];
...
function ScheduleView({data, mode = "full"}: { data: BusSchedule; mode?: "full" | "compact" }) {
    ...
    const {
        isGeneralSchedule, dayType, setDayType, direction, setDirection, activeSchedule, hours, nextBus, highlightedHour
    } = useScheduleLogic(data);

    return (<div className={isCompact ? "space-y-4" : "space-y-5"}>
        {!isCompact && (<RouteInfo details={data.routeDetails} featuredStops={data.featuredStops}/>)}

        {!isCompact && !isGeneralSchedule && (<DayTypeSelector current={dayType} onChange={setDayType}/>)}
        ...
        {!isCompact && (<div className="text-center text-[10px] text-gray-400 font-medium uppercase tracking-widest">
