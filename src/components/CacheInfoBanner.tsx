...
export const CacheInfoBanner: React.FC<CacheInfoBannerProps> = ({
    ...
    return (
        <div
            className="flex flex-wrap items-center justify-between gap-2 px-1 mb-4 text-xs text-slate-500 dark:text-slate-400 select-none">
          ...
          {cooldown.text}
            ...
            {/* Action Button: Dedicated Refresh with Glow when ready & Greyed-out when cooling down */}
            <div className="flex items-center gap-2">
                <button
                    type="button"
                    ...
                    className={`relative inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-black transition-all text-[11px] select-none ${
                        isRefreshing
                            ? "bg-blue-600/80 text-white cursor-wait border border-blue-400/40 shadow-xs"
                            : cooldown.isReady
                                ? "bg-blue-600 hover:bg-blue-700 text-white border border-blue-400/50 shadow-md shadow-blue-500/30 hover:shadow-lg hover:shadow-blue-500/50 ring-2 ring-blue-400/40 cursor-pointer active:scale-95 animate-pulse"
