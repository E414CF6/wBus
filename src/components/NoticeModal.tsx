    ...
    >
      ...
      >
        ...
        {selectedNoticeId ? (
          <div className="flex-1 flex flex-col overflow-hidden">
            {isDetailLoading ? (
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-4">
                {/* Meta Box */}
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/10 space-y-2.5">
                  <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-white leading-snug">
                    {detailData.title}
                  </h3>
                  <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-200/60 dark:border-white/10">
                    ...
                    <div className="space-y-1.5">
                      {detailData.files.map((file, idx) => (
                        ...
                        >
                          ...
                          <Download className="w-3.5 h-3.5 shrink-0 text-amber-600 dark:text-amber-400 group-hover:scale-110 transition-transform" />
                        </a>
                      ))}
                    </div>
                  </div>
                ...
                {(detailData.prevId || detailData.nextId) && (
                  <div className="p-3 rounded-2xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/10 space-y-1.5 text-xs">
                    {detailData.prevId && (
                      <div
            ...
            {/* Footer of Detail */}
            <div className="p-3.5 sm:p-4 border-t border-slate-200/80 dark:border-white/10 bg-slate-50/90 dark:bg-white/[0.03] flex items-center justify-between gap-3 shrink-0">
              <button
                type="button"
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Search & Tabs Row */}
            <div className="p-3 sm:p-4 border-b border-slate-200/80 dark:border-white/10 bg-white/50 dark:bg-[#121620]/50 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shrink-0">
              {/* Tab Pills */}
              <div className="flex items-center gap-1 p-0.5 rounded-xl bg-slate-100 dark:bg-white/[0.06] text-xs font-bold shrink-0">
                <button
                  type="button"
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 mb-1">
                        {item.isNotice ? (
                          <span className="px-2 py-0.5 rounded-md bg-amber-500 text-white font-black text-[10px] shrink-0 shadow-2xs">
                            공지
                          </span>
                        ) : (
                          <span className="font-mono font-bold text-slate-400 text-xs shrink-0 w-7">
                            {item.num}
                          </span>
                        )}
            ...
            {/* Pagination Controls */}
            {totalPages > 1 && (
              <div className="p-3 border-t border-slate-200/80 dark:border-white/10 bg-slate-50/90 dark:bg-white/[0.03] flex items-center justify-center gap-2 shrink-0">
                <button
                  type="button"
                  disabled={page <= 1}
