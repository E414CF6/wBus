...
function ArrivalItem({
    ...
    bus: BusStopArrival; onRouteChange?: (name: string) => void
}) {
    ...
    >
        <div className="flex flex-col items-start gap-1.5 overflow-hidden">
            <div className="flex items-center gap-2">
                ...
                <span
                    className="text-[10px] font-bold text-gray-400 dark:text-gray-500 truncate tracking-tight uppercase">
...
function ArrivalList({
                         loading, error, arrivalData, onRouteChange,
                     }: {
    loading: boolean; error: string | null; arrivalData: BusStopArrival[]; onRouteChange?: (routeName: string) => void;
}) {
    if (error) {
    ...
    if (!loading && arrivalData.length === 0) {
        return (<div className="p-4 text-center">
            <div
                className="py-10 bg-gray-50 dark:bg-white/5 rounded-[20px] border border-gray-100 dark:border-white/10">
                <div
                    className="mx-auto w-10 h-10 bg-gray-200 dark:bg-gray-800 text-gray-400 rounded-full flex items-center justify-center mb-3">
    ...
    return (<div className="max-h-80 overflow-y-auto custom-scrollbar p-3 sm:p-4 space-y-2">
...
export default function BusStopPopup({
                                         stopId, onRouteChange,
                                     }: {
    stopId: string; onRouteChange?: (routeName: string) => void;
}) {
    const {data: arrivalRawData, loading, error} = useBusArrivalInfo(stopId);

    const sortedArrivalData = useMemo(() => {
        return arrivalRawData ? [...arrivalRawData].sort((a, b) => a.arrtime - b.arrtime) : [];
    }, [arrivalRawData]);

    return (<div className="w-full min-w-65 sm:min-w-[320px] bg-transparent overflow-hidden">
        <ArrivalList
            loading={loading}
            error={error}
