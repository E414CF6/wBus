import {addComment, deleteComment, getComments, likeComment,} from "@entities/comment/commentService";
import {checkLikeRateLimit, checkPostRateLimit, getClientIp, hashClientIp,} from "@shared/lib/security";
...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json(
            {
                success: false,
                error: error instanceof Error ? error.message : "스퀘어 글 조회 실패",
            },
            {status: 500}
        );
    }
...
export async function POST(request: NextRequest) {
    try {
            ...
            return NextResponse.json(
                {
                    ...
                    error:
                        rateLimit.reason ||
                        "너무 빠르게 글을 작성하고 있습니다. 잠시 후 다시 시도해주세요.",
                },
                {
                    status: 429,
                    headers: {
        ...
        return NextResponse.json(
            {
                success: false,
                error: error instanceof Error ? error.message : "스퀘어 글 등록 실패",
            },
            {status: 400}
        );
    }
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        if (!id) {
            return NextResponse.json(
                {success: false, error: "대상 글 ID가 필요합니다."},
                {status: 400}
            );
        }
        ...
        if (action === "like") {
            ...
            if (!likeLimit.allowed) {
                return NextResponse.json(
                    ...
                    {
                        status: 429,
                        headers: {"Retry-After": String(likeLimit.retryAfterSec || 10)},
                    }
                );
            }
            ...
            if (!updated) {
                return NextResponse.json(
                    {success: false, error: "해당 글을 찾을 수 없습니다."},
                    {status: 404}
                );
            }
    ...
    try {
        ...
        if (!updated) {
            return NextResponse.json(
                {
                    success: false,
                    error: "해당 글을 찾을 수 없거나 삭제 권한이 없습니다.",
                },
                {status: 403}
            );
        }
