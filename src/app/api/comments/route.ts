...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({
            success: false, error: error instanceof Error ? error.message : "댓글 조회 실패",
        }, {status: 500});
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        const newComment = await addComment({
            author, content, routeNo, category, parentId, replyToAuthor, authorTag, replyToAuthorTag,
        });
        ...
        return NextResponse.json({
            success: false, error: error instanceof Error ? error.message : "댓글 등록 실패",
        }, {status: 400});
    }
}
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        if (!id) {
            return NextResponse.json({success: false, error: "대상 댓글 ID가 필요합니다."}, {status: 400});
        }
        ...
        return NextResponse.json({
            success: false, error: error instanceof Error ? error.message : "댓글 수정 실패",
        }, {status: 500});
    }
}
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (!id) {
            return NextResponse.json({success: false, error: "삭제할 댓글 ID가 필요합니다."}, {status: 400});
        }

        const updated = await deleteComment(id, authorTag);
        if (!updated) {
            return NextResponse.json({success: false, error: "댓글을 찾을 수 없거나 권한이 없습니다."}, {status: 404});
        }
        return NextResponse.json({success: true, comment: updated});
    } catch (error) {
        console.error("API DELETE /api/comments error:", error);
        return NextResponse.json({
            success: false, error: error instanceof Error ? error.message : "댓글 삭제 실패",
        }, {status: 500});
    }
}
