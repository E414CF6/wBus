"use client";

import React, {memo} from "react";
import {Bus, Calendar, Check, Clock, HelpCircle, type LucideIcon, MapPin, Search, X,} from "lucide-react";

import type {DayFilter, ShuttleTab} from "../../types";

// ============================================================================
// Constants & Configurations
// ============================================================================

const REVISION_DATE = "2026/08/21";
const ROUTE_SUMMARY =
    "여주역 · 만종역 · 원주고속터미널 · 원주역 · 시청사거리 · 세브란스 ↔ 연세대학교 미래캠퍼스";

interface ShuttleTabConfig {
    readonly id: ShuttleTab;
    readonly label: string;
    readonly icon: LucideIcon;
    readonly activeGradient?: string;
}

const DEFAULT_ACTIVE_TAB_GRADIENT =
    "bg-gradient-to-r from-teal-600 to-emerald-600 text-white shadow-md shadow-teal-700/20";
const STOPS_ACTIVE_TAB_GRADIENT =
    "bg-gradient-to-r from-blue-600 to-teal-600 text-white shadow-md shadow-blue-600/20";

const SHUTTLE_TABS: readonly ShuttleTabConfig[] = [
    {id: "inbound", label: "등교", icon: Clock},
    {id: "outbound", label: "하교", icon: Clock},
    {id: "stops", label: "정류장 지도", icon: MapPin, activeGradient: STOPS_ACTIVE_TAB_GRADIENT},
    {id: "guidelines", label: "이용 안내", icon: HelpCircle},
] as const;

interface DayFilterOption {
    readonly key: DayFilter;
    readonly label: string;
}

const DAY_FILTER_OPTIONS: readonly DayFilterOption[] = [
    {key: "ALL", label: "전체"},
    {key: "WEEKDAY", label: "평일 (월~금)"},
    {key: "SUNDAY", label: "일요일"},
] as const;

// ============================================================================
// Sub-components (Flattened Structure & Minimized Nesting)
// ============================================================================

interface ShuttleTitleBarProps {
    onClose: () => void;
}

const ShuttleTitleBar: React.FC<ShuttleTitleBarProps> = memo(({onClose}) => (
    <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
            <div
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-tr from-teal-600 via-emerald-600 to-teal-700 text-white flex items-center justify-center shadow-md shadow-teal-700/20 shrink-0">
                <Bus className="w-5 h-5" aria-hidden="true"/>
            </div>
            <div className="min-w-0">
                <h2 className="text-base sm:text-lg font-black text-slate-900 dark:text-white truncate flex items-center gap-2">
                    <span>셔틀버스</span>
                    <span
                        className="text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-500/30">
                        무료
                    </span>
                    <span
                        className="text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-lg bg-teal-50 dark:bg-teal-950/50 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-500/30">
                        {REVISION_DATE}
                    </span>
                </h2>
                <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                    {ROUTE_SUMMARY}
                </p>
            </div>
        </div>

        <button
            type="button"
            onClick={onClose}
            aria-label="닫기 (ESC)"
            title="닫기 (ESC)"
            className="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/[0.06] dark:hover:bg-white/[0.12] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white flex items-center justify-center transition-colors cursor-pointer shrink-0"
        >
            <X className="w-4 h-4" aria-hidden="true"/>
        </button>
    </div>
));
ShuttleTitleBar.displayName = "ShuttleTitleBar";

interface ShuttleNavTabsProps {
    activeTab: ShuttleTab;
    onTabChange: (tab: ShuttleTab) => void;
}

const ShuttleNavTabs: React.FC<ShuttleNavTabsProps> = memo(({activeTab, onTabChange}) => (
    <nav
        role="tablist"
        aria-label="셔틀버스 메뉴 탭"
        className="flex items-center gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-white/[0.06] border border-black/5 dark:border-white/5 overflow-x-auto no-scrollbar"
    >
        {SHUTTLE_TABS.map(({id, label, icon: Icon, activeGradient}) => {
            const isActive = activeTab === id;
            return (
                <button
                    key={id}
                    type="button"
                    role="tab"
                    aria-selected={isActive}
                    onClick={() => onTabChange(id)}
                    className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-xl font-black text-xs sm:text-sm transition-all cursor-pointer whitespace-nowrap ${
                        isActive
                            ? activeGradient ?? DEFAULT_ACTIVE_TAB_GRADIENT
                            : "text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
                    }`}
                >
                    <Icon className="w-3.5 h-3.5" aria-hidden="true"/>
                    <span>{label}</span>
                </button>
            );
        })}
    </nav>
));
ShuttleNavTabs.displayName = "ShuttleNavTabs";

interface ShuttleFilterBarProps {
    dayFilter: DayFilter;
    onDayFilterChange: (filter: DayFilter) => void;
    searchQuery: string;
    onSearchQueryChange: (query: string) => void;
}

const ShuttleFilterBar: React.FC<ShuttleFilterBarProps> = memo(({
                                                                    dayFilter,
                                                                    onDayFilterChange,
                                                                    searchQuery,
                                                                    onSearchQueryChange,
                                                                }) => (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-1">
        {/* Day filter pills */}
        <div className="flex items-center gap-1 flex-wrap text-xs" role="group" aria-label="운행일 필터">
            <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 mr-1 flex items-center gap-1">
                <Calendar className="w-3 h-3" aria-hidden="true"/>
                운행일:
            </span>
            {DAY_FILTER_OPTIONS.map((pill) => {
                const isSelected = dayFilter === pill.key;
                return (
                    <button
                        key={pill.key}
                        type="button"
                        aria-pressed={isSelected}
                        onClick={() => onDayFilterChange(pill.key)}
                        className={`px-2.5 py-1 rounded-xl font-bold transition-all cursor-pointer flex items-center gap-1 ${
                            isSelected
                                ? "bg-teal-500/20 text-teal-700 dark:text-teal-300 border border-teal-500/40"
                                : "bg-slate-100 hover:bg-slate-200 dark:bg-white/[0.05] dark:hover:bg-white/10 text-slate-600 dark:text-slate-400"
                        }`}
                    >
                        {isSelected && <Check className="w-3 h-3 text-teal-600 dark:text-teal-400" aria-hidden="true"/>}
                        <span>{pill.label}</span>
                    </button>
                );
            })}
        </div>

        {/* Search filter input */}
        <div className="relative min-w-[160px] sm:w-48">
            <Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"
                    aria-hidden="true"/>
            <input
                type="text"
                placeholder="출발지/경유지 검색..."
                aria-label="출발지 또는 경유지 검색"
                value={searchQuery}
                onChange={(e) => onSearchQueryChange(e.target.value)}
                className="w-full pl-8 pr-7 py-1 text-xs rounded-xl bg-slate-100 dark:bg-white/[0.05] border border-black/5 dark:border-white/5 focus:border-teal-500 focus:outline-hidden text-slate-900 dark:text-white placeholder-slate-400"
            />
            {searchQuery && (
                <button
                    type="button"
                    onClick={() => onSearchQueryChange("")}
                    aria-label="검색어 지우기"
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-white p-0.5 cursor-pointer"
                >
                    <X className="w-3 h-3" aria-hidden="true"/>
                </button>
            )}
        </div>
    </div>
));
ShuttleFilterBar.displayName = "ShuttleFilterBar";

// ============================================================================
// Main Component
// ============================================================================

export interface ShuttleHeaderProps {
    activeTab: ShuttleTab;
    onTabChange: (tab: ShuttleTab) => void;
    dayFilter: DayFilter;
    onDayFilterChange: (filter: DayFilter) => void;
    searchQuery: string;
    onSearchQueryChange: (query: string) => void;
    onClose: () => void;
}

export const ShuttleHeader: React.FC<ShuttleHeaderProps> = ({
                                                                activeTab,
                                                                onTabChange,
                                                                dayFilter,
                                                                onDayFilterChange,
                                                                searchQuery,
                                                                onSearchQueryChange,
                                                                onClose,
                                                            }) => {
    const isScheduleTab = activeTab === "inbound" || activeTab === "outbound";

    return (
        <header
            className="p-4 sm:p-6 border-b border-teal-500/20 flex flex-col gap-3 bg-white dark:bg-[#111622] shrink-0">
            <ShuttleTitleBar onClose={onClose}/>
            <ShuttleNavTabs activeTab={activeTab} onTabChange={onTabChange}/>
            {isScheduleTab && (
                <ShuttleFilterBar
                    dayFilter={dayFilter}
                    onDayFilterChange={onDayFilterChange}
                    searchQuery={searchQuery}
                    onSearchQueryChange={onSearchQueryChange}
                />
            )}
        </header>
    );
};
