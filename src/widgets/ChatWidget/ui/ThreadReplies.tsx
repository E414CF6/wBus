        ...
        <div className="mt-3 pl-6 sm:pl-10 space-y-2 border-l-2 border-slate-100 dark:border-white/5 animate-fadeIn">
            {replies.map((reply) => {
                ...
                return (<div
                    key={reply.id}
                    id={`comment-${reply.id}`}
                    className={`p-3 rounded-2xl border transition-all ${reply.isDeleted ? "bg-slate-50/50 dark:bg-white/[0.01] border-slate-200/40 dark:border-white/5 opacity-50" : "bg-slate-50/80 dark:bg-white/[0.02] border-slate-200/60 dark:border-white/5"}`}
                >
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                        <div className="flex items-center gap-2">
                            <div
                                className={`w-6 h-6 rounded-xl bg-gradient-to-tr ${getAvatarGradient(reply.author, reply.authorTag)} text-white flex items-center justify-center font-black text-[11px] shrink-0`}
                            >
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                {reply.authorTag && (<span className="text-[10px] font-mono text-slate-400">
                                            {reply.authorTag}
                                        </span>)}
                                {isReplyMine && (
                        ...
                        {/* Reply delete button */}
                        {isReplyMine && !reply.isDeleted && (<button
                    ...
                    <div className="flex items-center justify-between pt-1.5 pl-8 mt-1">
                        <button
                            ...
                            className={`px-2 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer ${isReplyLiked ? "text-rose-600 dark:text-rose-400 font-black" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}
                        >
                            <Heart
