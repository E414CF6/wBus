...
export const ThreadCard: React.FC<ThreadCardProps> = ({
    ...
    return (<div
        id={`thread-${thread.id}`}
        className={`p-4 rounded-3xl border transition-all ${thread.isDeleted ? "bg-slate-50/50 dark:bg-white/[0.01] border-slate-200/50 dark:border-white/5 opacity-60" : "bg-white dark:bg-[#151b28] border-slate-200/80 dark:border-white/5 shadow-xs hover:border-indigo-500/30 dark:hover:border-indigo-500/30"}`}
    >
        {/* Thread Header */}
        <div className="flex items-start justify-between gap-3 mb-2.5">
            <div className="flex items-center gap-2.5 min-w-0">
                <div
                    className={`w-9 h-9 rounded-2xl bg-gradient-to-tr ${getAvatarGradient(thread.author, thread.authorTag)} text-white flex items-center justify-center font-black text-sm shadow-xs shrink-0`}
                >
            ...
            <div className="flex items-center gap-1 shrink-0">
                ...
                {isMine && !thread.isDeleted && (<button
        ...
        <div
            ...
            {thread.isDeleted ? (<span className="italic text-slate-500 dark:text-slate-400 flex items-center gap-1">
                        <Ban className="w-3.5 h-3.5"/>
                        삭제된 글입니다.
                    </span>) : (renderRichContent(thread.content, onHashtagClick))}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 dark:border-white/5 pt-2 pl-11">
            <div className="flex items-center gap-2">
                ...
                <button
                    ...
                    className={`px-2.5 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${isLiked ? "bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 font-black" : "hover:bg-slate-100 dark:hover:bg-white/[0.06] text-slate-500 dark:text-slate-400"}`}
                >
        ...
        {inlineReplyTarget && isReplyOpen && (<InlineReplyComposer
        ...
        {/* Nested Replies List */}
        {replies.length > 0 && (<ThreadReplies
            replies={replies}
