...
export const ChatSidebar: React.FC<ChatSidebarProps> = ({
    ...
    return (<aside className="hidden lg:flex lg:col-span-4 flex-col gap-4 sticky top-4">
            {/* User Profile Card */}
            <div
                ...
                <div className="flex items-center gap-3 pt-1">
                    <div
                        className={`w-12 h-12 rounded-2xl bg-gradient-to-tr ${getAvatarGradient(authorName, userTag)} text-white flex items-center justify-center font-black text-lg shadow-md`}
                    >
                        {authorName.charAt(0) || "?"}
                    </div>
                    <div className="min-w-0 flex-1">
                        ...
                        {userTag && (<p className="text-xs font-mono text-slate-400 dark:text-slate-500">
                                #{userTag.replace(/^#+/, "")}
                            </p>)}
                    </div>
                </div>
            ...
            <div
                ...
                <div className="space-y-1.5 pt-1">
                    {trendingTags.length > 0 ? (trendingTags.map((t, idx) => {
                            const isSelected = selectedHashtag === t.tag;
                            return (<div
                                    key={t.tag}
                                    onClick={() => onHashtagClick(t.tag)}
                                    className={`flex items-center justify-between p-2 rounded-2xl transition-all cursor-pointer ${isSelected ? "bg-blue-600 text-white shadow-xs" : "hover:bg-slate-100 dark:hover:bg-white/5 text-slate-700 dark:text-slate-300"}`}
                                >
                                    <div className="flex items-center gap-2 min-w-0">
                                        <span
                                            className={`text-xs font-mono font-black ${isSelected ? "text-white" : idx === 0 ? "text-rose-500" : idx === 1 ? "text-amber-500" : "text-blue-500"}`}
                                        >
                                            #{idx + 1}
                                        </span>
            ...
            {topRankedThreads.length > 0 && (<div
                    ...
                    <div className="space-y-2 pt-1">
                        {topRankedThreads.map(({thread, replyCount}, idx) => (<div
                                key={thread.id}
                            ...
                            >
                                <div className="flex items-center justify-between gap-2">
                                    <span
                                        className={`px-1.5 py-0.2 rounded-md text-[9px] font-black ${idx === 0 ? "bg-amber-400 text-slate-900" : idx === 1 ? "bg-slate-300 text-slate-900 dark:bg-slate-600 dark:text-white" : "bg-amber-700/60 text-white"}`}
                                    >
                                        TOP {idx + 1}
                                    </span>
