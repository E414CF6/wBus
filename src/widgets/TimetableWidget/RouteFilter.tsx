    ...
    return (
        <div className="space-y-4 mb-6">
          ...
          <span
              className="text-xs font-bold text-slate-600 dark:text-slate-400 shrink-0 mr-1 flex items-center gap-1">
            <Layers
                className="h-3.5 w-3.5 text-indigo-500 dark:text-indigo-400 stroke-[2.4]"/> {UI_TEXT.TIMETABLE.MAIN_ROUTES_LABEL}
          </span>
        ...
        <span className="font-medium">
          {UI_TEXT.TIMETABLE.SEARCH_RESULTS_LABEL} <strong
            className="text-slate-900 dark:text-white font-mono font-bold">{totalFilteredCount}</strong>{UI_TEXT.TIMETABLE.ROUTE_COUNT(totalFilteredCount).replace(/^[0-9]+/, "")}
        </span>
