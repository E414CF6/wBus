        ...
        if (blobResult && blobResult.url) {
          ...
          if (res.ok) {
            ...
            if (data && Array.isArray(data.routes) && data.routes.length > 0) {
              console.log(
                `[Blob] Loaded ${data.routes.length} routes from Vercel Blob (${blobPath}).`
              );
              return data;
            }
          }
    ...
    for (const blobPath of [VERCEL_BLOB_PATH, LEGACY_BLOB_PATH]) {
      try {
        ...
        if (res.ok) {
          ...
          if (data && Array.isArray(data.routes) && data.routes.length > 0) {
            console.log(
              `[Blob] Loaded ${data.routes.length} routes from direct Blob URL (${blobPath}).`
            );
            return data;
          }
        }
...
export async function saveToVercelBlob(data: RouteDataset): Promise<boolean> {
  ...
  try {
    ...
    console.warn(
      "[Blob] Could not save to Vercel Blob (OIDC or BLOB_READ_WRITE_TOKEN required):",
      err instanceof Error ? err.message : err
    );
    return false;
  }
