...
function setup()
  return {
    ...
    classes = Sequence {
        'toll', 'motorway', 'ferry', 'restricted', 'tunnel', 'residential'
    },
...
function process_way(profile, way, result, relations)
  ...
  local data = {
   ...
   if data.highway == "service" or data.highway == "track" or data.highway == "path" or data.highway == "footway" or data.highway == "pedestrian" then
       return false
   end
  ...
  local way_name = way:get_value_by_key("name")
  if way_name and (way_name:find("길$") or way_name:find("번길") or way_name:find("안길") or way_name:find("골목")) then
      result.forward_speed = 3
      result.backward_speed = 3
      result.weight = result.weight * 10
  end

  -- Disable startpoint snapping for minor roads: prevents bus stops from snapping onto alleys
  if data.highway == "residential" or data.highway == "living_street" or data.highway == "unclassified" then
      result.is_startpoint = false
      result.forward_classes["residential"] = true
      result.backward_classes["residential"] = true
  end

  -- Width-based penalty: narrow roads (< 4m) are unsuitable for buses
  local width = tonumber(way:get_value_by_key("width"))
  if width and width < 4.0 then
      result.forward_speed = math.min(result.forward_speed or 5, 5)
      result.backward_speed = math.min(result.backward_speed or 5, 5)
  end

  -- Single-lane penalty for minor roads
  local lanes = tonumber(way:get_value_by_key("lanes"))
  if lanes and lanes <= 1 and data.highway ~= "motorway_link" and data.highway ~= "trunk_link" then
      result.forward_speed = math.min(result.forward_speed or 10, 10)
      result.backward_speed = math.min(result.backward_speed or 10, 10)
  end
